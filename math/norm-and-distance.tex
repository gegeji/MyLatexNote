...
\begin{theorem}
    $ \left|a^{T} b\right| \leq\|a\|\|b\| \quad $ for all $ a, b \in \mathbf{R}^{n} $
\end{theorem}
...
\begin{corollary}[用$ \ell_{2} $范数表示的柯西—施瓦茨不等式]
...
\begin{itemize}
    ...
    \item $\theta =0$：a和b为同向(aligned or parallel
    )的 $(a ^T  b=\| a \| \| b  \| )$. 
...
\begin{theorem}
    [positive definite]
$ \|a\| \geq 0 $ for all $ a, \quad\|a\|=0 \quad $ only if $ a=0 $
\end{theorem}

\begin{theorem}
    [homogeneous]
$ \|\beta a\|=|\beta|\|a\| \quad $ for all vectors $ a $, complex scalars $ \beta $
\end{theorem}
