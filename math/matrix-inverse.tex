...
\begin{definition}[$A$的左逆]
    ...
    $X$被称为 $ A $ 的\term{左逆}; 当左逆存在时，则称$A$是\term{可左逆}的;
\end{definition}
...
\begin{definition}[逆 $A^{-1}$]
    如果矩阵$A$存在左逆和右逆， 此时$X$称为矩阵的\term{逆},记作 $ A^{-1} $.当矩阵的逆存在时，则称矩阵$A$\term{可逆}.
\end{definition}
...
\begin{theorem}
    对于方阵 $ A \in \mathfrak{R}^{n \times n} $ ,以下条件都是等价的:
...
\begin{proof}
    ...
    \begin{itemize}
        \item 性质 $ ( {a}) $ 对任意矩阵 $ A \in \mathfrak{R}^{m \times n} $ 都成立 
        \item 性质$(b)$对方阵矩阵 $ A \in \mathfrak{R}^{n \times n} $ 都成立
        \item 对于性质 $ ( {c}) $ 与 $ ( {d}) $, 可利用 $ A^{T} $ 证明
    \end{itemize}
\end{proof}
...
\begin{proof}
    ...
    \begin{corollary}
        如果 $ A \in \mathfrak{R}^{m \times n} $有左逆，则有 $ m \geq n = r $. 
    ...
    当 $ b \in \mathfrak{R}^{m}, b \notin\left\{y \mid y=A x, x \in \mathfrak{R}^{n}\right\} $ 时（即$b$不在$A$的列空间，$ m \geq n  $时），线性方程组无解。  $ A x=b $ 至多一个解，如有解则 $ x=X b $ . 
\end{proof}
...
\begin{theorem}
    $(c)$: 矩阵 $ A \in \mathfrak{R}^{m \times n} $ 有右逆 $ X $, 则$A$行向量线性无关。
\end{theorem}
...
\begin{proof}
    ...
    即$ A^{T} \in \mathfrak{R}^{n \times m} $.
    
    根据定理“矩阵的行秩等于列秩”，$ A^{T} $的列向量线性无关，则矩阵 $ A $ 有$m$个线性无关列向量（行向量），即通过 Gram-Schmidt 正交化可得$m$个正交基。 

    $ \forall b \in \mathfrak{R}^{m} $, 有$ b \in\left\{y \mid y=A x, x \in \mathfrak{R}^{n}\right\} (m \leq n ) $, 方程$ A x=b $有解，其解为$ x=X b $. 
\end{proof}

\begin{corollary}
    如果$ A \in \mathfrak{R}^{m \times n} $有右逆，则有 $r= m \leq n  $. 
...
\begin{proof}
    假设 $ A \in \mathfrak{R}^{n \times n} $ 为方阵且列向量线性无关 
    
    \begin{equation} A=\left[a_{1}, a_{2}, \cdots, a_{n}\right] \end{equation}

    则对于\textbf{任意}向量 $  {b} \in \mathfrak{R}^{n} $, 则向量组 $ \left[a_{1}, a_{2}, \ldots, a_{n},  {b}\right] $ 线性相关，存 在不全为0的系数，使得以下等式成立
    \begin{equation}
    ...
    存在向量 $ c_{1}, \ldots, c_{n} \in \mathfrak{R}^{n} $,使得 
    \begin{equation}
    \begin{aligned}
        Ac _{1}&=e_{1}\\
         A c_{2}&=e_{2}\\
...
\begin{corollary}[Gram Matrix 可逆等价于$A$列线性无关]
    矩阵 $ A \in \mathfrak{R}^{m \times n},   {G}=A^{T} A $
...
\begin{definition}[Pseudo-inverse of Matrix $A (m \geq n)$ ]
    矩阵$A \in \mathfrak{R}^{m \times n} $ ,当 $ m \geq n $ 时，列向量线性无关，即 $ A $ 的 Gram矩阵可逆。$A$的伪逆 $ A^{+} $定义:
    \begin{equation}
...
\begin{theorem}
    矩阵$A \in \mathfrak{R}^{m \times n} $ ,当 $ m \geq n $ 时，$ A^{+} $为矩阵 $ A $ 的左逆:
\begin{equation}
...
\begin{definition}[Pseudo-inverse of Matrix $A (m \leq n)$ ]
...
\begin{theorem}
    矩阵 $  {A} \in \mathfrak{R}^{m \times n} $ ,当 $m \leq  {n} $ 时，伪逆 $ A^{\dagger} $ 为 $ A $ 的右逆
\begin{equation}
...
The new symbol $A^{+}$is needed when $r<m$ or $r<n$. Then $A$ has no two-sided inverse, but it has a \term{pseudo}inverse $A^{+}$with that same rank $r$ :
