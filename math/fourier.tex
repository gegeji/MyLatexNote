...
将周期函数按上述方式展开, 它的物理意义是很明确的,这就是把一个比较复杂的周期运动看成是许多不同频率的简谐振动的叠加。 在电工学上,这种展开称为\term{谐波分析},其中常数项 $ A_{0} $ 称为 $ f(t) $ 的\term{直流分量}, $ A_{1} \sin \left(\omega t+\varphi_{1}\right) $ 称为\term{一次谐波}(又叫做\term{基波}) $ , A_{2} \sin \left(2 \omega t+\varphi_{2}\right), A_{3} \sin \left(3 \omega t+\varphi_{3}\right), \cdots $ 依次称为\term{二次谐波}, \term{三次谐波},等等。
