...
惩罚未能满足约束条件, 引入\term{拉格朗日函数 (Lagrange Function)}
$$
...
\begin{problem}
    ...
    矩阵 $ \mathrm{A} \in \mathbb{R}^{m \times n}, \mathrm{C} \in \mathbb{R}^{p \times n} $, 向量 $ \mathrm{b} \in \mathbb{R}^{m}, d \in \mathbb{R}^{p} $
\end{problem}
...
\begin{itemize}
    \item 当 $ \mathrm{p}=0 $ 时，则其转化为无约束的最小二乘法问题。
    \item 当 $ \mathrm{A}=I, b=0 $ 时，则其为最小范数问题。
\end{itemize}
...
\begin{definition}[优化条件的Karush-Kuhn-Tucker(KKT)等式]
...
\begin{proof}
    假设 $ x $ 满足 $ C x=d,(\hat{x}, z) $ 满足KKT等式的定义.
    ...
    \end{aligned}$$
        ...
        $ \hat{x} $ 是唯一的，因为假设矩阵A列线性无关， C行线性无关，即
$$
\begin{aligned}
...
\begin{proof}
    ...
    由于 $ A^{T} \mathrm{~A} x+C^{T} \mathrm{z}=0 $, 则当 $ x=0 $ 时，有 $ \mathrm{z}=0 $ 。 
...
第一步的时间复杂度是$\left(2 m n^{2}\right. $ flops $ )$，第二步的时间复杂度是$(2 \mathrm{mn} $ flops $ )$，总时间复杂度为$ 2 m n^{2}+(2 / 3)(p+n)^{3} $ flops 。
...
假设 1(\ref{prop:assumption-1}) 保证了 $ A^{T} A+C^{T} C $ 是非奇异的，即存在以下 $ Q R $ 因子分解

$ \left[\begin{array}{l}A \\ C\end{array}\right] $ 列向量无关，则 
...
稳定性: QR分解避免直接计算 $ A^{T} \mathrm{~A}_{\circ} $。
