...
$ \lambda_{i} \in \mathfrak{R}, i \in {I}, u_{j} \in \mathfrak{R}^{+}, j \in J $称为\term{拉格朗日乘子(Lagrange Multipliers)} .
...
矩阵 $C \in \mathfrak{R}^{p \times n} $, 向量 $ d \in \mathfrak{R}^{p} $.
\end{problem}
...
\begin{problem}
    矩阵 $ C \in \mathfrak{R}^{2 \times 10} $, 向量 $ d \in \mathfrak{R}^{2} $ :
...
\begin{remark}行线性无关矩阵$A$伪逆的定义：
    矩阵 $  {A} \in \mathfrak{R}^{m \times n} $ ,当$m \leq  {n} $ 时，且行向量线性无关，则 $ A A^{T} $ 可逆。 $ A^{T} $ 列向量线性无关，即 $ \left(A^{T}\right)^{T} A^{T}= {A} A^{T} $ 非奇异；
    定义伪逆:
    \begin{equation}
...
对矩阵 $ C^{T} \in \mathfrak{R}^{n \times p} $ 进行 $ Q R $ 分解， 设$ C^{T}=Q R $
\begin{equation}
\begin{aligned}
...
\begin{problem}[分段多项式拟合问题]
    ...
    矩阵 $ {A} \in \mathfrak{R}^{m \times n}, {C} \in \mathfrak{R}^{p \times n} $, 向量 $ {b} \in \mathfrak{R}^{m}, d \in \mathfrak{R}^{p} $
\end{problem}
...
\begin{problem}
    假设样本点 $ \left(x_{1}, y_{1}\right), \ldots,\left(x_{N}, y_{N}\right) \in \mathfrak{R}^{2} $ ,有 \begin{equation} x_{1}, \ldots, x_{M} \leq a , x_{M+1}, \ldots, x_{N}>a\end{equation}
...
\begin{problem}[分段多项式拟合问题]
    假设样本点 $ \left(x_{1}, y_{1}\right), \ldots,\left(x_{N}, y_{N}\right) \in \mathfrak{R}^{2} $ ,有 $ x_{1}, \ldots, x_{M} \leq a $, $ x_{M+1}, \ldots, x_{N}>a $.
...
A \\
C
\end{array}\right] \in \mathfrak{R}^{(m+p) \times n}
\end{equation}
\end{proposition}

\begin{proposition}
    \label{prop:assumption-2}
    矩阵 $ C \in \mathfrak{R}^{p \times n} $行线性无关
...
引入拉格朗日函数
\begin{equation}
L(x, z)=\frac{1}{2}\|A x-b\|_{2}^{2}-z^{T}(d-C x), z \in \mathfrak{R}^{p}
\end{equation}
...
\hat{x} \\
z
\end{array}\right] \in \mathfrak{R}^{n+p}
\end{equation}
...
由矩阵 $ C=Q_{2} R \Rightarrow Q_{2}=C R^{-1}, Q_{2} $ 行线性无关可以推出，对于向量$u$假如满足$Q_{2}^{T} u = 0$，则$u=0$。
\begin{equation}
...
\begin{equation} \left[\begin{array}{cc}A^{T} A+C^{T} C & C^{T} \\ C & 0\end{array}\right]\left[\begin{array}{c}\hat{x} \\ w\end{array}\right]=\left[\begin{array}{c}A^{T} b \\ d\end{array}\right] \Rightarrow \tilde{R} w=\tilde{Q}^{T} Q_{1}^{T} b-\tilde{R}^{-T} d \end{equation}
...
\begin{problem}[等式约束优化问题]
    给定一个目标函数 $ f: \mathfrak{R}^{n} \rightarrow \mathfrak{R} $, 我们希望找到 $ {x} \in \mathfrak{R}^{n} $, 在满足约束条件 $ g({x})=0 $ 的前提下， 使得 $ f({x}) $ 有最小值
...
约束不等式 $ g({x}) \leq 0 $ 称为\term{原始可行性(primal feasibility)}, 据此我们定义\term{可行域(feasible region)} $ K=\{ {x} \in \mathfrak{R}^{n} \mid g({x}) \leq 0 \}$.
\end{problem}
...
\end{array}
\end{equation}
其中 $ \left(x_{1}, x_{2}\right) \in \mathfrak{R}^{2}, \alpha $ 为实数。
\end{problem}
...
\begin{definition}[点、（直）线、线段]
    $ x_{1} \neq x_{2} $ 是 $ \mathfrak{R}^{n} $ 中的两\term{点}, $ \theta \in \mathfrak{R} $, 那么 $ y=x_{2}+\theta\left(x_{1}-x_{2}\right) $ 表示穿过两点的\term{线}.

    当 $ 0 \leqslant \theta \leqslant 1 $ 时， $ y $ 是 $ x_{1} $ 到 $ x_{2} $ 的\term{线段},$ y $ 也可以表示成 $ y=\theta x_{1}+(1-\theta) x_{2} $.
\end{definition}

\begin{definition}[仿射集]
    一个集合 $ C \subseteq \mathfrak{R}^{n} $ 是\term{仿射集}如果其中任意两个不同的点的连线仍包含于 $ C $ .
\end{definition}

\begin{definition}[凸集]
    一个集合 $ C \subseteq \mathfrak{R}^{n} $ 是凸集， 如果任意两点之间的线段仍包含于 $ C $ .即 $ \forall x_{1}, x_{2} \in C $, 任意 $ 0 \leqslant \theta \leqslant 1 $, 有 $ \theta x_{1}+(1-\theta) x_{2} \in C $ .
\end{definition}
...
\begin{definition}[凸函数]
    一个函数 $ f: \mathfrak{R}^{n} \rightarrow \mathfrak{R} $ 是\term{凸函数}如果定义域是凸集而且对任意定义域的 $ x, y, 0 \leqslant \theta \leqslant 1 $ 有
\begin{equation}
...
$ x=\left(x_{1}, \ldots, x_{n}\right) $ 是\term{优化变量}, 函数 $ f_{0}: \mathfrak{R}^{n} \rightarrow \mathfrak{R} $ 是\term{目标函数}, 函数 $ f_{i}: \mathfrak{R}^{n} \rightarrow \mathfrak{R} $ 是\term{约束函数}. $ b_{i} $ 是约束。使得 $ f_{0}(x) $ 在约束条件下最小的 $ x^{*} $ 叫做\term{最优点}, 或问题的\term{解}.
\end{definition}
...
\begin{definition}[拉格朗日函数]
    给定一个凸优化问题， 拉格朗日算子是一个函数 $ \mathcal{L}: \mathfrak{R}^{n} \times \mathfrak{R}^{m} \times \mathfrak{R}^{p} \rightarrow \mathfrak{R} $, 定义为:
    \begin{equation}
    \mathcal{L}(x, \alpha, \beta)=f(x)+\sum_{i=1}^{m} \alpha_{i} g_{i}(x)+\sum_{i=1}^{p} \beta_{i} h_{i}(x)
    \end{equation}

    $ x \in \mathfrak{R}^{n} $ 叫做\term{主变量(primal variable)}. $ \alpha \in \mathfrak{R}^{m}, \beta \in \mathfrak{R}^{p} $ 统称对偶变量或拉格朗日乘子。
\end{definition}
...
括号内的 $ \theta_{\mathcal{P}}: \mathfrak{R}^{n} \rightarrow \mathfrak{R} $ 叫做\term{主目标(primal objective)}, 等号右边的没有限制的最小化问题叫做\term{主问题(primal problem)}. 用 $ x^{*} $ 表示\term{主问题的解},$ p^{*}=\theta_{\mathcal{P}}\left(x^{*}\right) $ 表示\term{主目标的最优值}.
\end{problem}
...
$ \theta_{\mathcal{D}}(\alpha, \beta): \mathfrak{R}^{m} \times \mathfrak{R}^{p} \rightarrow \mathfrak{R} $ 是\term{对偶目标(dual objective)},等号右边的有限制的最大化问题叫做\term{对偶问题}. 用 $ \left(\alpha^{*}, \beta^{*}\right) $ 表示\term{对偶问题的解}, $ d^{*}=\theta_{\mathcal{D}}\left(\alpha^{*}, \beta^{*}\right) $ 表示\term{对偶目标的最优值}.
\end{definition}
...
\begin{theorem}
    设 $ x^{*} \in \mathfrak{R}^{n}, \alpha^{*} \in \mathfrak{R}^{m}, \beta^{*} \in \mathfrak{R}^{p} $, 下列条件为 $ {KKT} $ 条件:
...
\begin{theorem} Let $z : \mathfrak{R}^n \rightarrow \mathfrak{R}$ be a differentiable objective function, $g_i:\mathfrak{R}^n \rightarrow \mathfrak{R}$ be differentiable constraint functions for $i = 1,\dots,m$ and $h_j:\mathfrak{R}^n \rightarrow \mathfrak{R}$ be differentiable constraint functions for $j=1,\dots,l$. If ${x}^* \in \mathfrak{R}^n$ is an optimal point satisfying an appropriate regularity condition for the following optimization problem:
...
then there exists $\lambda_1,\dots,\lambda_m \in \mathfrak{R}$ and $\mu_1,\dots\mu_l \in \mathfrak{R}$ so that:
...
\mu_j \in \mathfrak{R} \quad \text{for $j = 1,\dots,l$}
\end{aligned}
\right.\\
...
\begin{theorem} Let $z : \mathfrak{R}^n \rightarrow \mathfrak{R}$ be a differentiable concave (凹) function, $g_i:\mathfrak{R}^n \rightarrow \mathfrak{R}$ be differentiable convex functions for $i = 1,\dots,m$ and $h_j:\mathfrak{R}^n \rightarrow \mathfrak{R}$ be affine functions for $j=1,\dots,l$. Suppose there are $\lambda_1,\dots,\lambda_m \in \mathfrak{R}$ and $\mu_1,\dots\mu_l \in \mathfrak{R}$ so that:
...
\begin{equation}\text{Dual Feasibility}:\left\{
\begin{aligned}
