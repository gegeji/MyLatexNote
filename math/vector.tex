...
\begin{definition}[标量与向量的乘法]
    \begin{equation} \beta a=\left[\begin{array}{c}\beta a_{1} \\ \vdots \\ \beta a_{n}\end{array}\right] \end{equation}
\end{definition}
...
\begin{definition}[Complex Numbers]
    ...
    $ {j}=\sqrt{-1} $ (more common notation is $ i $ or $ j $ ), $ \alpha $ is the \term{real} part of $ x $, denoted $ \operatorname{Re} x $, $ \beta $ is the \term{imaginary} part, denoted $ \operatorname{Im} x $
\end{definition}
...
$ \theta \in[0,2 \pi) $ is the \term{argument} (\term{phase angle}) of $ x $ (notation: $ \arg x $ ). $ e^{{j} \theta} $ is complex exponential.
\end{theorem}
